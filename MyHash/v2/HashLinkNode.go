...
func NewHashLinkMap(Cap int) *HashLinkMap {
	h := new(HashLinkMap)
	h.Cap = Cap
	//h.Data = [Cap]*ListNode{}
...
func (h *HashLinkMap) Insert(k, v int) {
	i := hash(h.Cap, k)
...
func (h *HashLinkMap) Search(k int) (error, int) {
	i := hash(h.Cap, k)
	current := h.Data[i]
	if current == nil {
